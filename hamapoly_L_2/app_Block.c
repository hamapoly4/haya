    ...
    while(1)
    {
        ...
        switch(r_state)
        {
            ...
            case START: // ********************************************************************
                if(direction < 40)
                {
                    turn = Run_getTurn_change(turn, 73, 0.5);
            ...
            case CURVE:   // ********************************************************************
                motor_ctrl_alt(50, 27, 0.1);                // 指定速度まで減速しつつ右曲がりに前進
            ...
            case RETURN:   // ********************************************************************
                if(distance < temp + 1170)
                {
                    if(direction < 250)
                    {
                        turn = Run_getTurn_change(turn, 42, 0.4);
                        motor_ctrl_alt(50, turn, 0.4);
                    }
                    else
                    {
                        turn = Run_getTurn_change(turn, 0, 0.4);
                        motor_ctrl(50, turn);
                    }
                }
                else                                // 指定距離に到達した場合
                {
                    ...
                    if(direction < 320)
                    {
                        turn = Run_getTurn_change(turn, 90, 0.4);
                        motor_ctrl_alt(15, turn, 0.1);        // 減速して右曲がりに走行
                    }
                    else
                    {
                        motor_ctrl(15, 0);
            ...
            case END:   // ********************************************************************
                ...
                {
                    turn = Run_getTurn_sensorPID(rgb.r, 50);    // PID制御を用いて旋回値を取得
